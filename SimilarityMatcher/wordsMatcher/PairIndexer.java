...
static String _separator =" ";//le s�parateur d'�l�ments, par d�faut un espace
//TODO: probablement mieux d'avoir recours aux ensembles de caract�res pr�d�finis 
...
//On utilise une MultiHashMap pour construire une table des diff�rentes positions (offset) d'un m�me mot (string)
//ex: a::{0,4,6}
	{
	    ...
	    //� la fin de cette boucle, table contient la position de chaque mot, sequenceSet l'ensemble sans doublons
	    //des mots de la String
	    
	    MultiHashMap mhm = new MultiHashMap();
	    Enumeration e = table.keys();
	    //on r�cup�re les offsets, pour les stocker dans mhm sous forme de values: les string seront les keys
	    while(e.hasMoreElements())
	        {
	        	...
	        	//on obtient une Collection de positions associ�e � chaque mot-key
	        }
	    return mhm;
	}
	...
	public void indexPair(String[] one, String[] two)
	    {
	    	...
	    	//le keySet permet de lister les s�quences, alors que mhm.values retourne les positions
	...
	public String printIntersectionString(Vector intersectionVector)
	    {
	    	...
	    	if(_element1Size > _element2Size){
	    		...
	    		//� la fin de ces 2 tests, tailleTab contient la taille maxi du tableau, tailleShortestTab
	    		//la taille du plus petit des segments
	    	    
	    	//String[] posAndWords = new String[tailleTab];
	    	ArrayList posAndWords = new ArrayList(tailleTab);//une ArrayList est pr�f�rable: plus flexible
	    	...
	    	while(e.hasMoreElements()){//on parcourt le vecteur d'intersection
	    	    ...
	    	    posList.addAll(longestMhm.getCollection(word));//on r�cup�re pour le mot courant la liste des positions
	    	    ...
	    	    for(int ind = 0; ind < posList.size(); ind++){//boucle de remplissage de posAndWords
	    	        posInt = (Integer)posList.get(ind);//on r�cup�re la position du mot trait� dans le tableau de positions
	    	        pos = posInt.intValue();
	    	        ...
	    	        if(shortestPosList.size() < 2){//Pas de r�p�tition
	    	            ...
	    	            else{//D�placement d'�l�ment: shortestMhm < intersectionVector
	    	                if(aligned(word,pos,word,posShortest)){//2
	    	                    //posAndWords.set(pos,"(MOV:" + word + posShortest + "=>" + pos + ")");
	    	                    posAndWords.set(pos,word);//TODO: r�gler probl�me plus tard
	    	            		}//2
	    	            	}
	    	            ...
	    	            if(aligned(word,pos,word,posShortest)){//tant que les mots sont align�s on �crit dans posAndWords
    	                    posAndWords.set(pos,word);
    	            		}
	    	            else{	    	                
	    	        ...
	    	        if(posShortest == pos){//si les 2 formes sont align�es
	    	            //posAndWords[pos] = word;
	    	            System.out.println("align: " + word);
	    	            posAndWords.set(pos,word);
	    	        	}	    	        
	    	        else if(posShortest != pos){//si les 2 formes ne sont pas align�es
	    	            //System.out.println("word: " + word);
	    	            if(posAndWords.contains(word)){//si le mot est r�p�t�, donc � plusieurs endroits de la cha�ne
	    	            ...
	    	            //ET si posAndWords ne contient pas d�j� word (pas un MOV)
	    	        	}*/
	    	    }//fin: for
	    	}//fin: while
	    	//� la fin de cette boucle, on a un tableau posAndWords rempli de mots de intersectionVector aux positions 
	    	//trouv�es dans le longestMhm
	...
	public boolean aligned(String a, int pos1, String b, int pos2){//retourne true si les 2 cha�nes sont align�es: m�me String/m�me position
	    boolean align = false;
...
public static void main(String[] args) {
    	...
    	String[] tata = {"le","chat","mange","la","souris"};
    	 String[] toto = {"le","tr�s","gros","chat","poilu","mange","goul�ment","la","toute","petite","souris","grise"};*/
 ...
 * @param _element1 Le _element1 � param�trer.
 */
 ...
 * @param _mhm2 Le _mhm2 � param�trer.
 */
